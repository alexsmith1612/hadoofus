...
static struct hdfs_error	_datanode_read_init(struct hdfs_datanode *d, off_t bloff, off_t len);
static struct hdfs_error	_datanode_read(struct hdfs_datanode *d, off_t len, int fd, off_t fdoff,
				void *buf, ssize_t *nread);
static struct hdfs_error	_datanode_read_blocking(struct hdfs_datanode *d, off_t bloff, off_t len, int fd,
				off_t fdoff, void *buf);
...
static struct hdfs_error	_datanode_write_blocking(struct hdfs_datanode *d, const void *buf, int fd,
				off_t len, off_t fdoff);
...
hdfs_datanode_new(struct hdfs_object *located_block, const char *client,
	int proto, bool crcs, struct hdfs_error *error_out)
{
	...
	hdfs_datanode_init(d, located_block, client, proto, crcs);
...
hdfs_datanode_write(struct hdfs_datanode *d, const void *buf, size_t len)
{
	...
	return _datanode_write_blocking(d, buf, -1/*fd*/, len, -1/*fdoff*/);
}
...
hdfs_datanode_write_file(struct hdfs_datanode *d, int fd, off_t len, off_t offset)
{
	...
	return _datanode_write_blocking(d, NULL/*buf*/, fd, len, offset);
}
...
EXPORT_SYM struct hdfs_error
hdfs_datanode_read_nb_init(struct hdfs_datanode *d, off_t bloff, off_t len)
{
	return _datanode_read_init(d, bloff, len);
}
...
hdfs_datanode_read(struct hdfs_datanode *d, size_t off, size_t len, void *buf)
{
	...
	return _datanode_read_blocking(d, off, len, -1/*fd*/, -1/*fdoff*/, buf);
}
	...
	int fd, off_t fdoff)
{
	...
	return _datanode_read_blocking(d, bloff, len, fd, fdoff, NULL/*buf*/);
}
...
static struct hdfs_error
_datanode_read_blocking(struct hdfs_datanode *d, off_t bloff, off_t len, int fd,
	off_t fdoff, void *buf)
{
	struct hdfs_error error;
	ssize_t nr = 0;
	...
	error = _datanode_read_init(d, bloff, len);
	if (hdfs_is_error(error))
		goto out;
...
static struct hdfs_error
_datanode_write_blocking(struct hdfs_datanode *d, const void *buf, int fd,
	off_t len, off_t fdoff)
{
	struct hdfs_error error;
	ssize_t nw = 0, na = 0;
