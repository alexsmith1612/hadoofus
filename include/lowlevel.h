...
//
// The argument crcs indicates whether or not crcs should be sent when performing
// a datanode write or whether or not crcs should be requested/validated when
// performing a datanode read.
...
// operation and the total number of bytes to be read.
//
...
struct hdfs_error	hdfs_datanode_read_nb_init(struct hdfs_datanode *d, off_t bloff, off_t len);
