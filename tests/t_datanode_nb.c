	...
	do {
		...
		do {
			...
			case ST_AB:
				...
				hdfs_datanode_init(&fctxp->dn, obj, client, dn_proto, _i/*crcs*/);
			...
			case ST_GB:
				...
				hdfs_datanode_init(&fctxp->dn, lb, client, dn_proto, _i/*crcs*/);
			...
			if (fctxs[i].st == ST_WR) { // Writing the files
				...
				if (!hdfs_is_error(err)) {
					...
					if (fctxs[i].rtot == TOWRITE) {
						...
						fctxs[i].blkidx++;
						lb = fctxs[i].bls->ob_val._located_blocks._blocks[fctxs[i].blkidx];
						hdfs_datanode_init(&fctxs[i].dn, lb, client, dn_proto, _i/*crcs*/);
